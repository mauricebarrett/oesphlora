...
# Ensure we have enough colors for the groups
final_colors <- c(
  "grey",
  cb_palette[seq_len(min(num_groups - 1, length(cb_palette)))]
)
...
for (metric in metrics) {
  ...
  p <- ggplot(metric_data, aes(
    x = .data[[primary_variable]],
    y = value,
    fill = .data[[primary_variable]]
  )) +
    ...
    theme(
      ...
      axis.line.y = element_line(
        linetype = 1, linewidth = 0.5, colour = "black"
      ),
