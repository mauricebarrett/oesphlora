...
nmds_data$treatments_used <- factor(nmds_data$treatments_used,
    levels = c("Control", sort(setdiff(unique(nmds_data$treatments_used), "Control")))
)

print(levels(nmds_data$treatments_used))
...
num_treatments <- length(levels(nmds_data$treatments_used))
