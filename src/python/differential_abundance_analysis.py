...
def linda_daa_taxon(
    ...
    for group_a, group_b in pairwise_combinations:
        ...
        # Subset OTU table base on metadata
        taxon_table_fil = taxon_table[metadata_subset.index]

        # Skip if there are fewer than 4 samples (columns)
        if taxon_table.shape[1] < 4:
            print(
                f"Error: only {taxon_table.shape[1]} samples found (minimum 4 required)"
            )
            continue
